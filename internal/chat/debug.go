...
func (d *debugImpl) Connect(channel string) {
	d.callback(Event{Type_: Connect})
}

// Disconnect implements Chat.
func (d *debugImpl) Disconnect() {
}

// OnEvent implements Chat.
func (d *debugImpl) OnEvent(callback func(e Event)) {
	d.callback = callback
}
